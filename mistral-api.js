...
class MistralEmotionAnalyzer {
    ...
    async analyzeTextSentiment(text) {
        ...
        try {
            const response = await fetch(this.baseUrl, {
                ...
                body: JSON.stringify({
                    model: "mistral-large-latest", // Using a suitable Mistral model
                    messages: [
                        {
                            role: "user",
                            content: prompt
            ...
            if (data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content) {
                ...
                // Find JSON within the response (might be surrounded by text)
                const jsonMatch = content.match(/\{[\s\S]*\}/);
                if (jsonMatch) {
                    const result = JSON.parse(jsonMatch[0]);
                    return result;
                } else {
                    throw new Error('Could not extract JSON from Mistral API response');
                }
            } else {
    ...
    async analyzeLocationEmotions(location, newsFeeds, cctvImages = [], audioClips = []) {
        ...
        // Since Mistral API doesn't support image or audio analysis directly,
        // we'll use default analysis for these components
        const imageAnalyses = cctvImages.map(() => this.getDefaultFaceAnalysis());

        const audioAnalyses = audioClips.map(() => this.getDefaultAudioAnalysis());
